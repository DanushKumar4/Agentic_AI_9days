...
@tool
def send_fee_notice(student_name: str, balance: float) -> str:
    """
    Send a fee-balance notice to a student.

    Use this tool ONLY when the user explicitly asks you to INFORM,
    NOTIFY, or SEND the student's fee balance to them.

    Do NOT use this tool when the user is merely asking for,
    checking, viewing, or retrieving the student's fee balance.
    """
    print(f"NOTICE SENT to {student_name}: fee balance = ₹{balance}")
    return "Notice sent successfully."

@tool
def always_fail() -> str:
    """
    A test tool that intentionally fails every time.
    Used to verify the circuit breaker.
    """
    raise RuntimeError("Intentional failure for circuit-breaker test")
